...
async def detect_screenshot(screenshot: UploadFile = File(...)):
    ...
    try:
        ...
        if actual_amount_val is not None:
            ...
            # If mismatch > 10.00, test if an obscured digit (e.g., '6' or '0' misread for '8') resolves to expected pricing
            if abs(actual_amount_val - expected_amount) > 10.00:
                ...
                for search_digit, replace_digit in [('6', '8'), ('0', '8')]:
                    if search_digit in s_val:
                        ...
                        if abs(candidate_val - cand_expected) <= 10.00:
        ...
        # Check if amount difference is within ₹10.00 buffer tolerance
        ...
        # Amount mismatch check with ₹10.00 buffer tolerance for live market price fluctuations
        if expected_amount is not None and actual_amount_val is not None and abs(actual_amount_val - expected_amount) > 10.00:
            ...
            reasons_list.append(f"Amount mismatch! Screenshot has {amount_str}, but expected integer multiple of live price + 18% GST (nearest expected amount: ₹{expected_amount:.2f} for {num_fractions} fractions, exceeding ₹10.00 buffer tolerance)")
