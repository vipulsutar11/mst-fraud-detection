...
def test_verify_api(image_path, buyer_id="BUYER-99", fraction_id="FRAC-LIVE", num_fractions=1, expected_datetime="2026-07-16T10:41"):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test_api.py <path_to_image> [buyer_id] [expected_datetime_iso]")
        print("Example: python test_api.py uploads/old_transaction.png BUYER-99 2026-07-16T10:41")
    else:
        img = sys.argv[1]
        buyer = sys.argv[2] if len(sys.argv) > 2 else "BUYER-99"
        dt = sys.argv[3] if len(sys.argv) > 3 else "2026-07-16T10:41"
        test_verify_api(img, buyer_id=buyer, expected_datetime=dt)
